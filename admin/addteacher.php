...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            ...
            $stmt = $db->prepare("
                INSERT INTO users (username, email, password_hash, full_name, phone, user_type, account_status, created_at) 
                VALUES (?, ?, ?, ?, ?, 'teacher', 'active', NOW())
            ");
            $stmt->execute([$username, $email, password_hash($password, PASSWORD_DEFAULT), $full_name, $phone]);
            $user_id = $db->lastInsertId();
            
            // Insert into user_profiles table
            $stmt = $db->prepare("
                INSERT INTO user_profiles (user_id, education_level, address, created_at) 
                VALUES (?, ?, ?, NOW())
            ");
            $stmt->execute([$user_id, $education_level, $address]);
            
            // Insert into teachers table (if exists)
            $stmt = $db->prepare("
                INSERT INTO teachers (user_id, specialization, experience_years, salary, hire_date, status) 
                VALUES (?, ?, ?, ?, NOW(), 'active')
            ");
            $stmt->execute([$user_id, $specialization, $experience_years, $salary]);
...
function setLoading(loading) {
    ...
    if (loading) {
        ...
        submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin me-1"></i>Adding Teacher...';
        submitBtn.disabled = true;
    } else {
        form.classList.remove('loading');
        submitBtn.innerHTML = '<i class="fas fa-save me-1"></i>Add Teacher';
